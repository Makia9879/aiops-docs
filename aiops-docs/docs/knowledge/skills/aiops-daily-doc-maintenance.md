...
3. 提炼关键词，在整个 workspace 里搜索受影响的内容
4. 确定哪些文档需要更新：

| 变更类型 | 更新范围 |
|---------|---------|
| 产品需求或能力变更 | `prd/` |
| 架构、模块、依赖、数据流变更 | `architecture/` |
| 接口、CLI、配置、协议变更 | `specs/` |
| 架构决策变更 | `adr/` |
| 业务流程、操作流程变更 | `workflows/` |
| 影响人类阅读体验 | `guides/docs/` |

5. 跨文档同步更新——一个接口改了，specs、workflows、architecture、guides 里关于它的描述都要一致
6. 把已处理的 pending 记录移到 `archived/YYYY-MM-DD.md`，从 `pending.md` 里删掉
7. 运行审查清单，确认没有遗漏
8. 按治理等级决定是否自动提交
