...
AIOps 知识治理按输入来源和使用方向分成四个场景，但它们共享同一套知识结构：
...
不管从哪个入口进入，使用的都是同一套 `.aiops/projects/<project>/` 知识结构。这意味着一个项目可以先从历史入库开始梳理，后续开发前用文档召回辅助研发，代码变更后再用维护流程持续更新。中间不需要"切换系统"或"重建文档"。
