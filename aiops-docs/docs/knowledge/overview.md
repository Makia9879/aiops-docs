...
.aiops/projects/<project>/
  project.yaml        # 项目元信息（产品域、治理等级、语言）
  README.md           # 知识库索引（只做导航）
  open-questions.md   # 待确认的问题
  prd/                # 为什么做、服务谁、不做什么
  architecture/       # 系统边界、模块职责、数据流
  specs/              # 接口、配置、协议、行为约束
  adr/                # 已确认的架构决策和取舍
  workflows/          # 端到端业务流程
  guides/             # 给人读的连续叙事站点
...
五类文档各有明确的写法和边界（具体见三个场景的文档），但它们在治理上是**一个整体**。维护时不是"更新某篇文章"，而是"检查这个变更对整个项目知识库的影响"。
...
Reading 层的更新是有方向的：不是手动维护两套内容，而是 canonical 层变更后同步更新 reading 层。保证人看到的信息和 agent 召回的信息来自同一份事实。
...
- **不追求文档完美**。第一轮只建立 agent 进入项目所需的最低知识闭环，之后持续迭代。不是"全部写完才开始用"。
- **不绑定特定工具**。技能和知识结构是标准化的，CLI 支持 Claude Code 和 Codex，未来可以扩展到更多平台。
- **不对代码做假设**。Agent 维护文档的依据是源码、配置、测试和 Hook 记录，不是"看起来应该是这样"。
