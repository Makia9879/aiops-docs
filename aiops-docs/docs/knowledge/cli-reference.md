...
```bash
aiops install [options]
```

**安装了什么：**

- 7 个 AIOps 治理技能 → `~/.agents/skills/` 和 `~/.codex/skills/`
- 3 个辅助工具 → `~/.aiops/tools/`
...
```bash
# 只装技能，跳过工具链
aiops install --with none

# 只装部分工具
aiops install --with codegraph,trellis
...
| 选项 | 类型 | 默认值 | 说明 |
|-----|------|-------|------|
| `--docs-repo <path>` | string | 无 | 指向 AIOps 文档仓库的路径（必填） |
